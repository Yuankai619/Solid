...
function StreamJoinedMessageCard({ data }) {
    // useEffect(() => {
    ...
    const { score, senderId, isAnonymous, content } = data;
    const avatarUrl = senderId?.avatarUrl;
    const userName = senderId?.userName;
    const isMyMessage = senderId._id === userId  //比對message的userID是不是==自己的userID
    console.debug("senderId, userId", senderId._id, userId);
    ...
    const [anonymousState, setAnonymousState] = useState(isAnonymous);
            ...
            <Card variant='outlined'>
                <CardContent>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        <Avatar sx={{ mx: "10px", width: '28px', height: '28px' }} src={avatarUrl} />
                        ...
                        {isShowScore &&
                            <Button aria-label="scroeState"
                                ...
                                sx={{
                                    background: score == 'correct' ? correctEnable : incorrectEnable,
                                    '&:hover': {
                                        background: score == 'correct' ? correctEnable : incorrectEnable, // 确保鼠标悬浮时的背景色与点击时相同
                                    },
                                }}
